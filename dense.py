...
from keras.layers import (Conv3D, BatchNormalization, AveragePooling3D, concatenate, Lambda,
                          Activation, Input, GlobalAvgPool3D)
from metrics import invasion_acc, invasion_precision, invasion_recall, invasion_fmeasure
...
def data_padding(data,mode,sigma):
     ...
     for data_count in range(0,np.size(data,0)):
         if mode == 0:   #x轴翻转
             ...
             data_back = np.flip(data,1)
              ...
              data_back = np.rot90(data,sigma)#旋转90*ratate_beta度
         elif mode == 3:#z轴
